...
(async () => {
  ...
  try {
    ...
    const prices = {
      ...
      cacheWrite: 0.3 / 1_000_000, // Mismo que output
    };

    console.log("\n💰 SIMULACIÓN DE COSTOS: Gemini 1.5 Flash (plan pago)\n");
    console.log(
      "Propósito".padEnd(25) +
        " | Calls |   Entrada   |   Salida   | Cache Read | Cache Write |  TOTAL",
    );
    ...
    const sorted = results.sort((a, b) => {
      const costA =
        (a.inputTokens || 0) * prices.input +
        (a.outputTokens || 0) * prices.output +
        (a.cacheReadTokens || 0) * prices.cacheRead;
      const costB =
        (b.inputTokens || 0) * prices.input +
        (b.outputTokens || 0) * prices.output +
        (b.cacheReadTokens || 0) * prices.cacheRead;
      return costB - costA;
    });
    ...
    for (const r of sorted) {
      ...
      console.log(
        `${purpose} | ${calls} | ${input} | ${output} | ${cacheR} | ${cacheW} | ${total}`,
      );
    }
    ...
    const grandTotal =
      `$${(totalInputCost + totalOutputCost + totalCacheReadCost + totalCacheWriteCost).toFixed(6)}`.padStart(
        8,
      );
    console.log(
      `${totalLabel} | ${totalCalls} | ${totalInputDisplay} | ${totalOutputDisplay} | ${totalCacheRDisplay} | ${totalCacheWDisplay} | ${grandTotal}\n`,
    );
    ...
    console.log(
      `  Por día:   ${callsPerDay} llamadas × $${costPerDay.toFixed(6)} = $${costPerDay.toFixed(2)}`,
    );
